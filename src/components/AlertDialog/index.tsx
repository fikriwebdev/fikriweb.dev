...
>((props, ref) => {
  ...
  return (
    <RadixAlert.Trigger asChild ref={ref} className={clsm(className)} {...rest}>
      {children}
    </RadixAlert.Trigger>
  );
});

const AlertDialogPortal: React.FC<RadixAlert.AlertDialogPortalProps> = (
  props
) => {
...
>((props, ref) => {
  ...
  return (
    <RadixAlert.Overlay
      ref={ref}
      className={clsm(
        "fixed inset-0 bg-black/30 animate-alertOverlayShow",
        className
      )}
      {...rest}
    />
